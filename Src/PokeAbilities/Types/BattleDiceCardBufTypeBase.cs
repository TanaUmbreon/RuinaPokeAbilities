using System;
using PokeAbilities.ResultOnlyPassives;
using UnityEngine;
...
namespace PokeAbilities.Types
{
    ...
    {
        // Memo:
        //   タイプ一致とタイプ相性は与ダメージ量を割合で増減させる。
        //   与ダメージ量を割合で増減させる場合はBeforeGiveDamageメソッドで行うのがベスト。
        //   BattleDiceCardBufクラスではこのメソッドが無いのでBattleUnitBufクラスに委譲させている。
        //   BattleUnitBufクラスはOnUseCard(バトルページ使用時)メソッドで付与している。
        ...
        public override void OnUseCard(BattleUnitModel owner)
        {
            try
            {
                ...
                Log.Instance.DebugOnBattleDiceCardBufWithCaller("バトルページを使用してタイプ攻撃キャラクター状態を付与しました。", this);
            }
            catch (Exception ex)
            {
        ...
        public override void OnRoundEnd()
        {
            ...
            Destroy();
            Log.Instance.DebugOnBattleDiceCardBufWithCaller("バトルページ状態を取り除きました。", this);
        }
        ...
        private class BattleUnitBuf_TypeAttacker : BattleUnitBuf
        {
            ...
            public override void BeforeGiveDamage(BattleDiceBehavior behavior)
            {
                try
                {
                    Log.Instance.DebugOnBattleUnitBufWithCaller("バトルダイスでダメージを与えようとしています。", this);
            ...
            {
                Destroy();
                Log.Instance.DebugOnBattleUnitBufWithCaller("タイプ攻撃キャラクター状態を取り除きました。", this);
            }
        }
    }
}
