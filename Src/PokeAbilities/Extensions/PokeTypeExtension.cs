...
namespace PokeAbilities.Extensions
{
    ...
    {
        /// <summary>
        /// この値のタイプを、それと等価なタイプ系状態に変換して返します。
        /// </summary>
        /// <param name="type">変換するタイプ。</param>
        /// <returns><paramref name="type"/> と同じタイプの状態。</returns>
        public static BattleUnitBufTypeBase ToBattleUnitBuf(this PokeType type)
        {
            switch (type)
            {
        ...
        /// <summary>
        /// この値のタイプを、それと等価なタイプ系バトル ページ状態に変換して返します。
        /// </summary>
        /// <param name="type">変換するタイプ。</param>
        /// <param name="isPermanent">幕をまたいでも永続的にタイプ付与する事を示す値。省略した場合はその幕でのみタイプ付与します。</param>
        /// <returns><paramref name="type"/> と同じタイプのバトル ページ状態。</returns>
        public static BattleDiceCardBufTypeBase CreateCardBuf(this PokeType type, bool isPermanent = false)
        {
            switch (type)
            {
                case PokeType.Normal:
